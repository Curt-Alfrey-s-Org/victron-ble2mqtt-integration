...
- Starts **`victron`**, **`homeassistant`**, **`autoheal`** ([willfarrell/autoheal](https://hub.docker.com/r/willfarrell/autoheal)), and optionally **`tools`** (Watchtower only) via Compose — workloads use **`restart: unless-stopped`**; **`healthcheck`** + **`autoheal`** restart **wedged** containers labeled **`autoheal=true`**. Set **`AUTOHEAL_CONTAINER_LABEL=autoheal`** (the label name). The entrypoint filter is `${AUTOHEAL_CONTAINER_LABEL}=true`; setting the env to `autoheal=true` would look for `autoheal=true=true` and never restart ([docker-entrypoint](https://github.com/willfarrell/docker-autoheal/blob/master/docker-entrypoint)).
- **`victron_ble2mqtt` healthcheck** — `victron_ble2mqtt.liveness.check()` (system-info heartbeat + BLE scanner ok file + BLE publish heartbeat; default `BLE_PUBLISH_MAX_AGE_SEC=600`). Legacy **`ha-watchdog.timer`** stays off (`ENABLE_HA_WATCHDOG=0`).
...
- **Dockge:** Stacks appear under **`/opt/stacks`** (`victron`, `homeassistant`, `autoheal`, `tools`, **`sungold`** when `ENABLE_SUNGOLD=1`, **`bms-supervisor`** when `ENABLE_BMS_SUPERVISOR=1`). Compose **`include`** points at files in your git checkout — edit Compose in the repo, then **Compose → Update** in Dockge or re-run **`sudo bash scripts/deploy.sh`** to refresh wrappers.
...
- **BMS supervisor (optional):** Read-only VE.Direct USB sidecar — **`docs/PI4_BMS_SOFTWARE.md`**. Default off (`ENABLE_BMS_SUPERVISOR=0`). Does not replace a pack BMS.
