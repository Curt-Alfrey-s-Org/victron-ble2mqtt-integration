...
|--------|---------|---------|
| **Charge** | `site_charge_power`, `victron_pack_charge_power`, `sungold_cart_charge_power` | Energy into storage. **T2/KU Victron** from MPPT/jumper (shunt charge power). **SPH cart** when `sungold_cart_charge_power` > 0. **Not load.** |
| **Load** | `site_total_load_power`, `site_end_use_load_power`, `ku_stack_load_power` | Energy used, not stored on Victron packs. **End-use** = SPH AC out (Pi4, dump, vent). **KU stack load** = cart charging from KU Renogy -> SPH path (same W as cart charge; load **on T2/KU buses**, not house AC out). **T2 MPPT DC load** included in total. **Victron pack charge is never load.** |
| **Measured hop (not a load tile)** | `trailer_outlet_power`, `ku_renogy_ac_load_power` | KU AC **into** SPH AC-in. When cart is **not** charging, watts here that are not stack load -> `sph_ac_in_unmatched_power` (+ conversion loss in `site_load_unaccounted_power`). |
...
- **Trailer outlet** = AC-in meter; if cart **Batt A** ~0, those AC-in watts are **SPH AC-in unmatched**, not missing load and not Victron charge.
