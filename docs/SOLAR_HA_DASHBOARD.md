...
| KU outlet to SPH | EM16 + helpers that are **not** SPH LCD/Modbus: **Trailer CT A3**, B3 breaker, KU Renogy AC est (`max(CT, UTI V×A)`), UTI V×A (SPH `AC INPUT V×A`, not a watt register -- SPH rejected `grid/power`), AC out V×A (cross-check vs LCD `INV OUTPUT LOAD KW`), Cart to AC out, Sungold conversion loss. SPH does not publish AC-in watts ([SUNGOLD_SPH302480A.md](SUNGOLD_SPH302480A.md)). |
