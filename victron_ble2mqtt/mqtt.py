...
class VictronMqttDeviceHandler:
    ...
    def publish(
        ...
        *,
        ...
        # find the correct handler class
        if handler is None:
            ...
            self.handler_map[mac_address] = handler

        data_dict = generic_device.data_dict

        try:
            handler.publish(data_dict=data_dict, rssi=rssi)
        except Exception:
            logger.exception('Failed to publish for %s', mac_address)
